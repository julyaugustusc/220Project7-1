...
void addStudent(string name_input, int rollNo_input, studentType type, int credits);
void displayList();
...
void executeAction(char c)
{
	...
	case 'a':	// add student
		...
		// I commented out the code that checks with searchStudent
		// To make it so a student with the same name can be added.
		//studentFound = searchStudent(name_input);
		//if (studentFound == NULL)
		//{
			addStudent(name_input, rollNo_input, type, credits_input);
	...
	case 'r':		// remove a student
		...
		getline(cin, name_input);
		studentFound = searchStudent(name_input);
		if (studentFound != NULL) //FIXED SO THIS WILL WORK
		{
...
int isValidInput(int rollNo, int credits)
{
	if (rollNo < 1 || rollNo > 999)			//will return 0 if anything not met
		return 0;
	else if (credits < 0 || credits > 90)
		return 0;
	else
		return 1;
...
int totalCredits(string name)
{
	...
	while (tempList != NULL) {			//parse through name and find, use operator+ to find total credits. 

		if(tempList->student->getName() == name){
			
			totalCredits = *tempList->student + totalCredits;
		}
...
void moreThan80Credits()
{
	...
	Container *newList = new Container();	//new container to start
	Container *nLParse = newList;
	Container *studentToRemove = NULL;		//for deletion
	// add code here
	while (tempList != NULL) {

		if (newList->student == NULL) {		//if at the beginning of new list
			if (tempList->student->getCredits() >= 80) {
				newList->student = tempList->student;		//use of operator=
			}
		}
		else {
			if (tempList->student->getCredits() >= 80) {
				nLParse->next = new Container();		//create new container first
				nLParse->next->student = tempList->student; // fill student pointer

				nLParse = nLParse->next; // move to next
			}
		}

		tempList = tempList->next; //move to next
	}

	nLParse = newList; // set to beginning

	while (nLParse != NULL) { // could have done above, felt more comfortable doing it separate.
		nLParse->student->displayInfo();
		nLParse = nLParse->next;
	}

	nLParse = newList;  // set to beginning

	while (newList != NULL) {		//doesn't matter if newList or nLParse
		
		studentToRemove = newList;
		newList = newList->next;		// move head of list to 2nd node	
		delete studentToRemove->student;
		delete studentToRemove;			//delete both things
...
// I unmodified it so the code would work properly and I commented out the code that checks with searchStudent
// To make it so a student with the same name can be added.
Student* searchStudent(string name_input)
{
...
void addStudent(string name_input, int rollNo_input, studentType type, int credits_input)
{
	Container* tempList = list;				// work on a copy of 'list'
	...
	if (type == undergrad)
		newListMember->student = new Undergrad(name_input, rollNo_input, type, credits_input);
	else
		newListMember->student = new Grad(name_input, rollNo_input, type, credits_input);
...
void save(string fileName)
{
	...
	if (myfile.is_open())
	{
		...
		while (tempList != NULL)
		{
			...
			myfile << tempList->student->getCredits() << endl;
			tempList = tempList->next;
		}
		myfile.close();
	}
...
void load(string fileName)
{
	...
	if (myfile.is_open())			// true if file exists
	{
		...
		for (int i = 0; i < count; i++)
		{
			...
			std::getline(myfile, temp);
			credits_read = stoi(temp);

			// create new student node depending on student type
			if (type_read == undergrad)
				newListMember->student = new Undergrad(name_read, rollNo_read, type_read, credits_read);
			else
				newListMember->student = new Grad(name_read, rollNo_read, type_read, credits_read);
